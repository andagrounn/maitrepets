...
export async function fulfillOrder(orderId) {
  ...
  // Mark as fulfilling BEFORE calling Printful to prevent duplicate submissions
  try {
    ...
    // If update fails the order is likely already being fulfilled
    return;
  }

  // Re-check after marking — guard against race condition
  const fresh = await prisma.order.findUnique({ where: { id: orderId } });
  if (fresh?.printfulId) {
    await logger.info('fulfillment', `Order already submitted to Printful (race guard)`, { orderId });
    return;
  }
