...
import { createPrintfulOrder, PRODUCT_VARIANTS, EXTRA_COPY_VARIANTS, getVariantByColor } from '@/lib/printful';
...
export async function fulfillOrder(orderId) {
  ...
  if (order.printfulId) {
    await logger.info('fulfillment', `Order already submitted to Printful`, { orderId });
    return;
  }
  ...
  // 'paper-*' types → thin canvas; everything else → framed poster (color-aware)
  ...
  const product = isPaper
    ...
    : getVariantByColor(order.productType, order.frameColor || 'black');
  ...
  const recipient = {
    ...
    email:        'hello@maitrepets.com', // store email — Printful internal use only, not customer-facing
  };

  // Atomic compare-and-swap: only claim the order if it's still in 'paid' state with no printfulId.
  // If two concurrent calls race here, only one will match the WHERE condition and proceed.
  const claimed = await prisma.order.updateMany({
    where: { id: orderId, status: 'paid', printfulId: null },
    data:  { status: 'fulfilling', updatedAt: new Date() },
  });
  if (claimed.count === 0) {
    await logger.info('fulfillment', `Order already claimed by another process`, { orderId });
    return;
  }
  ...
  try {
    ...
    await logger.info('fulfillment', `Order submitted to Printful`, { orderId, printfulId });
    ...
    await logger.error('fulfillment', err.message, { orderId, recipient });
