...
export async function fulfillOrder(orderId) {
  ...
  // 'paper-*' and 'canvas-*' types → thin canvas; everything else → framed poster (color-aware)
  const isPaper  = order.productType?.startsWith('paper-');
  const isCanvas = order.productType?.startsWith('canvas-');
  const product  = (isPaper || isCanvas)
    ? (EXTRA_COPY_VARIANTS[order.productType.replace(/^(paper|canvas)-/, 'poster-')] ?? EXTRA_COPY_VARIANTS['poster-16x20'])
