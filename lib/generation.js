...
const STYLE_SCENARIOS = {
  enhanced: [
    { scene: 'photographed against a clean neutral studio backdrop with a soft professional key light gently separating the subject, natural shadow fall, no props', mood: 'calm and natural', title: 'Studio Portrait' },
    { scene: 'in warm north-window light beside a white wall, the background falling into a clean shallow-focus gradient, no distractions', mood: 'serene and intimate', title: 'Window Light' },
    { scene: 'outdoors in soft golden-hour light, a gently blurred natural background of green foliage, professional shallow depth-of-field', mood: 'warm and vibrant', title: 'Golden Hour' },
    { scene: 'against a fine-art seamless grey paper backdrop, a large octabox softbox at 45 degrees creating a classic Rembrandt lighting triangle on the face', mood: 'professional and striking', title: 'Octabox Portrait' },
    { scene: 'in a bright airy studio with white bounce cards filling shadows, pristine white background, crisp catchlights in the eyes', mood: 'clean and vivid', title: 'High-Key Studio' },
  ],
  gothic: [
...
export function buildFinalPrompt(petDesc, styleConfig, styleKey) {
  ...
  const isEnhanced = styleKey === 'enhanced';

  const prompt = isEnhanced
    ? `Professional pet portrait photograph: ${petDesc}. ` +
      `${scenario.scene}. Mood: ${scenario.mood}. ` +
      `FRAMING: head and upper chest only — tight portrait crop, no full body, no legs or tail. ` +
      `The subject is a REAL ANIMAL. ZERO humans, humanoid figures, or people anywhere in the image. ` +
      `CRITICAL — preserve every individual detail: exact facial structure, distinctive markings, ` +
      `fur colors and patterns, eye color, ear style — this specific animal must be fully recognizable. ` +
      `Render faithfully as: ${styleConfig.prompt}. ` +
      `STRICTLY NO TEXT, watermarks, logos, or signatures anywhere in the image.`
...
// ─── gpt-image-1 ──────────────────────────────────────────────────────────────
export async function generateWithGPTImage(prompt, sourceImageUrl = null, quality = 'high') {
  ...
  if (sourceImageUrl) {
    try {
      console.log(`[pipeline] generating with gpt-image-1 (edit mode — reference image, quality: ${quality})...`);
  ...
  console.log(`[pipeline] generating with gpt-image-1 (quality: ${quality})...`);
...
export async function runGenerationPipeline(imageUrl, styleKey, quality = 'high') {
  ...
  console.log(`[pipeline] active model: ${activeModel}, quality: ${quality}`);
  ...
  if (activeModel === 'flux') {
    ...
    try {
      generatedUrl = await generateWithGPTImage(finalPrompt, imageUrl, quality);
    } catch (e) {
