...
// ─── Size mapping ─────────────────────────────────────────────────────────────
// Maps product key → { gpt: GPT-image-1 size string, flux: aspect_ratio string }
// GPT-image-1 supports: 1024x1024 (1:1), 1024x1536 (2:3 portrait), 1536x1024 (3:2 landscape)
const SIZE_MAP = {
  'poster-8x10':  { gpt: '1024x1280', flux: '4:5'  }, // 4:5 — gpt fallback: 1024x1536
  'poster-9x12':  { gpt: '1024x1536', flux: '3:4'  }, // 3:4
  'poster-11x14': { gpt: '1024x1536', flux: '4:5'  }, // ~4:5
  'poster-12x12': { gpt: '1024x1024', flux: '1:1'  }, // square
  'poster-12x16': { gpt: '1024x1536', flux: '3:4'  }, // 3:4
  'poster-12x18': { gpt: '1024x1536', flux: '2:3'  }, // 2:3
  'poster-16x16': { gpt: '1024x1024', flux: '1:1'  }, // square
  'poster-16x20': { gpt: '1024x1280', flux: '4:5'  }, // 4:5
  'poster-18x24': { gpt: '1024x1536', flux: '3:4'  }, // 3:4
  'poster-20x28': { gpt: '1024x1536', flux: '5:7'  }, // 5:7
  'poster-20x30': { gpt: '1024x1536', flux: '2:3'  }, // 2:3
  'poster-24x32': { gpt: '1024x1536', flux: '3:4'  }, // 3:4
  'poster-24x36': { gpt: '1024x1536', flux: '2:3'  }, // 2:3
};
const DEFAULT_SIZE = { gpt: '1024x1536', flux: '2:3' };

function getSizeConfig(productKey) {
  return SIZE_MAP[productKey] || DEFAULT_SIZE;
}
...
export function buildFinalPrompt(petDesc, styleConfig) {
  ...
  const prompt =
    ...
    `The subject is an ANIMAL — not a human — fully transformed into this character role. ` +
    ...
    `ABSOLUTELY NO text, words, letters, numbers, titles, watermarks, signatures, captions, or labels anywhere in the image.`;
...
// ─── gpt-image-1 ──────────────────────────────────────────────────────────────
export async function generateWithGPTImage(prompt, sourceImageUrl = null, gptSize = '1024x1536') {
...
// ─── Flux fallback ─────────────────────────────────────────────────────────────
export async function generateWithFlux(prompt, sourceImageUrl = null, fluxRatio = '2:3') {
  console.log('[pipeline] generating with flux-1.1-pro...');
  const input = {
    prompt,
    aspect_ratio:      fluxRatio,
    output_format:     'jpg',
    output_quality:    95,
  ...
  let out;
  try {
    out = await replicate.run('black-forest-labs/flux-1.1-pro', { input });
  } catch (e) {
    // If image_prompt is rejected (e.g. too small), retry without it
    if (sourceImageUrl && e.message?.toLowerCase().includes('image_prompt')) {
      console.warn('[flux] image_prompt rejected, retrying without reference image:', e.message);
      const { image_prompt, image_prompt_strength, ...inputWithout } = input;
      out = await replicate.run('black-forest-labs/flux-1.1-pro', { input: inputWithout });
    } else {
      throw e;
    }
  }
...
export async function runGenerationPipeline(imageUrl, styleKey, productKey = 'poster-16x20') {
  const styleConfig = STYLE_PROMPTS[styleKey];
  if (!styleConfig) throw new Error(`Unknown style: ${styleKey}`);

  const { gpt: gptSize, flux: fluxRatio } = getSizeConfig(productKey);
  console.log(`[pipeline] size for ${productKey}: gpt=${gptSize} flux=${fluxRatio}`);
  ...
  let generatedUrl;
  try {
    generatedUrl = await generateWithGPTImage(finalPrompt, imageUrl, gptSize);
  } catch (e) {
    console.warn('[gpt-image-1 failed — falling back to flux]', e.message);
    const fluxUrl = await generateWithFlux(finalPrompt, imageUrl, fluxRatio);
